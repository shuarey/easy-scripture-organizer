...
import {
  ...
  insertUserLanguage,
  setDeletedByName,
} from 'services/dbLanguageService';
...
export default function SettingsScreen() {
  ...
  const JSONlanguages = languageTranslationJSON.map((languageBlock, index) => ({
    id: index,
  ...
  useEffect(() => {
    ...
    getAllUserLanguages(db)
      .then((languages) => {
        ...
        const matchedIds = JSONlanguages.filter((jsonLang) =>
          languages.some((dbLang) => dbLang.name === jsonLang.name && dbLang.deleted === 'N')
        ).map((lang) => lang.id);
  ...
  function handleSelectedItemsChange(selectedIds: number[]) {
    setSelectedItems(selectedIds);
  }

  function handleButtonPress() {
    const selectedLanguages = JSONlanguages.filter((lang) => selectedItems.includes(lang.id));
    const dbLangMap = new Map(
      dbLanguages.filter((lang) => lang.deleted === 'N').map((lang) => [lang.name, lang])
    );
    ...
    dbLanguages.forEach((dbLang) => {
      const stillSelected = selectedLanguages.some((lang) => lang.name === dbLang.name);
    ...
    <Container>
      <ScreenContent title="Settings" scrollViewEnabled={false}>
        ...
        <MultiSelect
          ...
          hideSubmitButton={true}></MultiSelect>
